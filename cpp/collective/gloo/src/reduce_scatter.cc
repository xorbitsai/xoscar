...
void reduce_scatter(const std::shared_ptr<gloo::Context> &context,
    ...
    gloo::ReduceScatterHalvingDoubling<T> algorithm(
        context, dataPtrs, size, recvElems, fn);
    algorithm.run();

    memcpy(reinterpret_cast<T *>(recvbuf),
           inputbuf.data(),
...
void reduce_scatter_wrapper(const std::shared_ptr<gloo::Context> &context,
    ...
    switch (datatype) {
        case glooDataType_t::glooInt8:
            reduce_scatter<int8_t>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        case glooDataType_t::glooUint8:
            reduce_scatter<uint8_t>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        case glooDataType_t::glooInt32:
            reduce_scatter<int32_t>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        case glooDataType_t::glooUint32:
            reduce_scatter<uint32_t>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        case glooDataType_t::glooInt64:
            reduce_scatter<int64_t>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        case glooDataType_t::glooUint64:
            reduce_scatter<uint64_t>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        case glooDataType_t::glooFloat16:
            reduce_scatter<gloo::float16>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        case glooDataType_t::glooFloat32:
            reduce_scatter<float_t>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        case glooDataType_t::glooFloat64:
            reduce_scatter<double_t>(
                context, sendbuf, recvbuf, size, recvElems, reduceop);
            break;
        default:
